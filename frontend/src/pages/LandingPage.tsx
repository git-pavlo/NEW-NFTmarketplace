...
import { TrendingUp, Users, Layers, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function LandingPage({ onNavigate }: LandingPageProps) {
  ...
  const [stats, setStats] = useState({ volume: 0, users: 0, collections: 0 });
  ...
  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + featuredNFTs.length) % featuredNFTs.length);
  };
    ...
    <div className="min-h-screen pt-16">
      ...
      <section className="relative py-20 px-4 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            <motion.h1
              className="text-5xl md:text-7xl mb-6 neon-text"
            ...
            >
              <motion.button
                ...
                className="button-glow px-8 py-4 rounded-2xl bg-gradient-to-r from-[#8a6aff] to-[#38bdf8] text-white"
              >
                Explore Marketplace
              </motion.button>
              <motion.button
                ...
                className="px-8 py-4 rounded-2xl glassmorphism hover:border-[rgba(138,106,255,0.5)] transition-all"
              >
                Create NFT
              </motion.button>
            </motion.div>
          ...
          <div className="relative">
                    ...
                    <div className="glassmorphism rounded-3xl overflow-hidden">
                      <div className="grid md:grid-cols-2 gap-8 p-8">
                        ...
                        <div className="flex flex-col justify-center">
                          <h3 className="text-3xl mb-4 neon-text">{nft.name}</h3>
                          <p className="text-gray-300 mb-6">{nft.description}</p>
                          <div className="flex items-center justify-between mb-6">
                            <div>
                              <p className="text-sm text-gray-400">Current Price</p>
                              <p className="text-2xl neon-text">{nft.price} ETH</p>
                            </div>
                          ...
                          <motion.button
                            ...
                            className="button-glow w-full py-4 rounded-2xl bg-gradient-to-r from-[#8a6aff] to-[#38bdf8] text-white"
                          >
                            View Details
                          </motion.button>
                        </div>
            ...
            <div className="flex items-center justify-center gap-4 mt-6">
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={prevSlide}
                className="p-3 rounded-full glassmorphism hover:border-[rgba(138,106,255,0.5)]"
              >
                ...
                {featuredNFTs.map((_, index) => (
                  <motion.button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      currentSlide === index
                        ? 'w-8 bg-gradient-to-r from-[#8a6aff] to-[#38bdf8]'
                        : 'bg-gray-600'
                    }`}
                  />
                ))}
      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mockNFTs.slice(0, 6).map((nft, index) => (
              <motion.div
                key={nft.id}
                initial={{ opacity: 0, y: 20 }}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br ${color} mb-4`}
      >
        {icon}
      </motion.div>
      <motion.h3
